...
def load_models():
    ...
    try:
        ...
        # Load the 'base' model. It will be downloaded automatically if not present.
        whisper_model = whisper.load_model("base")
        logger.info("Whisper model loaded successfully")

        logger.info("Loading Emotion Recognition model...")
        # Load the local model using a relative path
        emotion_pipeline = pipeline(
            "text-classification",
            model="emotion_english_distilroberta_model",
            top_k=None
        )
...
def transcribe_audio_file(audio_path):
    ...
    if not ffmpeg_path:
        logger.error("ffmpeg.exe not found in PATH. Please ensure it is installed and the shell is fully restarted.")
        return None
...
def transcribe_endpoint():
    ...
    try:
        ...
        # Create a temporary file path and ensure it's closed before use
        ...
        try:
            ...
            text = transcribe_audio_file(tmp_filename)
        finally:
...
def transcribe_base64_endpoint():
    ...
    try:
        ...
        try:
            ...
            # Transcribe from the now-closed file
            text = transcribe_audio_file(tmp_filename)
        finally:
            # Clean up the temporary file
            os.unlink(tmp_filename)
