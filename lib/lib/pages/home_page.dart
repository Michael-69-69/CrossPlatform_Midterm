...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  Future<void> _onPickMedia() async {
    ...
    // Handle "analyze image" intent directly
  ...
  void _onSpeechProcessed(
    ...
    // Local navigation commands (bypass Gemini)
    ...
    // If culturalResponse is provided, speak it and skip Gemini
    if (culturalResponse != null && culturalResponse.isNotEmpty) {
      ...
      return;  // ← Prevent multiple replies by returning early
    }

    // Otherwise, use Gemini for intent routing
    ...
    if (intent == 'general:chat') {
      // General chat: Send text + image to Gemini
      String geminiResponse = await _geminiService.generateContent(
        text: text,
        imageFile: imageFile,
        emotions: emotions,
      ...
      // Navigation intents: Just navigate, no Gemini call
      _routeByIntent(intent);
    }
  }
  ...
  void _routeByIntent(String intent) {
    switch (intent) {
      ...
      case 'email:send':
        ...
        break;
      case 'notes:create':
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Notes feature coming soon!')));
        break;
      case 'attendance:view':
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Attendance feature coming soon!')));
        break;
      case 'fitness:start':
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Fitness feature coming soon!')));
        break;
      default:
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // 🔹 Replaced ModelViewer with simple animated placeholder
                  Expanded(
                    child: Center(
